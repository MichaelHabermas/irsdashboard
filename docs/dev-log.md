...
- **2026-05-07**: Removed automated git hooks (prior hook-runner package + lint-staged); rely on explicit `pnpm lint`, `pnpm format`, and CI. Deleted hooks directory under repo root, dropped root `prepare`/deps, removed Docker hook-suppression env lines from Dockerfiles.
